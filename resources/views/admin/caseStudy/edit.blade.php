            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <div>
                            <h1 class="page-title mb-0">Edit Case Study</h1>
                            <p class="page-subtitle mb-0">Update Case Study information</p>
                        </div>
                        <div>
                            <a href="{{ route('admin.caseStudy.index') }}" class="btn btn-light">
                                <i class="fas fa-arrow-left me-2"></i> Back to CaseList
                            </a>
                        </div>
                    </div>
                    ...
                    <div class="card-body">
                        ...
                        <form id="blogForm" action="{{ route('admin.caseStudy.update', $CaseStudy->id) }}" method="POST"
                            ...
                            <div class="row">
                                <div class="col-lg-12">
                                    ...
                                    <div class="form-section">
                                        <h3 class="section-title">Case Study Content</h3>
                                            ...
                                            <select class="form-control @error('category_id') is-invalid @enderror" 
                                                ...
                                                @foreach($categories as $category)
                                                    <option value="{{ $category->id }}" 
                                                        {{ old('category_id', $CaseStudy->category_id ?? '') == $category->id ? 'selected' : '' }}>
                                                        {{ $category->name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        ...
                                        <!-- Title Field -->
                                        <div class="mb-4">
                                            <label for="title" class="form-label required">Case Study Title</label>
                                            <input type="text"
                                                class="form-control form-control-lg @error('title') is-invalid @enderror"
                                                id="title" name="title" placeholder="Enter an engaging Case Study title"
                                        ...
                                        <!-- Description Field -->
                                        <div class="mb-4">
                                            <label for="description" class="form-label required">Case Study Description</label>
                                            <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description"
                                                rows="4" placeholder="Write a compelling description that summarizes your Case Study post" required>{{ old('description', $CaseStudy->description) }}</textarea>
                                            @error('description')
                                                <div class="error-message">{{ $message }}</div>
                                            @enderror
                                        </div>
                                        ...
                                        <div class="mb-4">
                                            <label for="content" class="form-label required">Case Study Content</label>
                                            <textarea id="content" name="content" class="form-control summernote @error('content') is-invalid @enderror">{{ old('content', $CaseStudy->content) }}</textarea>
                                            @error('content')
                                                <div class="error-message">{{ $message }}</div>
                                            @enderror
                                        </div>
                                    ...
                                    <div class="mb-4">
                                        ...
                                        <div class="d-flex align-items-center">
                                            <select class="form-control @error('status') is-invalid @enderror"
                                                ...
                                                <option value="draft"
                                                    {{ old('status', $CaseStudy->status) == 'draft' ? 'selected' : '' }}>Draft
                                                </option>
                                                <option value="published"
                                                    {{ old('status', $CaseStudy->status) == 'published' ? 'selected' : '' }}>
                                                    Published</option>
                                            </select>
                                        </div>
                                        @error('status')
                                    ...
                                    <div class="form-section">
                                        ...
                                        @if ($CaseStudy->featured_image)
                                            <div class="mb-3">
                                                ...
                                                <img src="{{ asset($CaseStudy->featured_image) }}" alt="Current Featured Image"
                                                    class="current-image">
                            ...
                            <div class="action-buttons">
                                <div class="d-flex justify-content-between align-items-center">
                                    ...
                                    <div class="d-flex gap-2">
                                        <a href="{{ route('admin.caseStudy.index') }}" class="btn btn-outline-secondary">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#resetBtn').on('click', function() {
                ...
                $('#title').val('{{ $CaseStudy->title }}');
                $('#slug').val('{{ $CaseStudy->slug }}');
                $('#description').val('{{ $CaseStudy->description }}');
                $('.summernote').summernote('code', `{!! $CaseStudy->content !!}`);
                $('#status').val('{{ $CaseStudy->status }}');
                updateStatusBadge();
